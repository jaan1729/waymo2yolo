...
#from https://github.com/shinya7y/WaymoCOCO.git
...
def main():
    ...
    parser.add_argument('--version_name', required=True, type=str)
    parser.add_argument('--image_filename_prefix', default=None, type=str)
    parser.add_argument('--skip_write_image', action='store_true')
    parser.add_argument('--ann_dirname', default='annotations', type=str)
    parser.add_argument('--ann_filename', required=True, type=str)
    ...
    image_dir = os.path.join(args.work_dir,'images', args.version_name)
    ann_dir = os.path.join(args.work_dir, args.ann_dirname)
    label_dir = os.path.join(args.work_dir,'labels', args.version_name)
    os.makedirs(image_dir, exist_ok=True)
    os.makedirs(ann_dir, exist_ok=True)
    os.makedirs(label_dir, exist_ok=True)
    ann_path = os.path.join(ann_dir, args.ann_filename)
    ...
    waymo_converter.write_coco(ann_path, json_indent=args.json_indent)
    print('converting to darknet.....')
    #From COCO we need to convert to darknet format
    ...
    for item in data['images']:
        ...
        outfile = open(os.path.join(label_dir,'%s.txt'%(file_name[:-4])), 'a+')
        for item2 in value:
            ...
            box = item2['bbox']
            bb = convert_bbox((width,height),box)
